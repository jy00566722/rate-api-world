<template>
  ...
  <script setup>
    ...
    onBeforeMount(()=>{
      chrome.storage.local.get(["currency_list_now","exchange_rate_obj","exchange_rate_crypto_obj"],(result)=>{
        ...
        // console.log(country_list_status.value)
    ...
    const searchInput = () => {
      ...
      country_list_status.value.forEach((el)=>{
        // if(el.searchText.includes(searchKey.value)|| 
        //    el.searchText.includes(searchKey.value.toUpperCase())){
        //    el.status = 'ALREADY'
        // }else{
        //   el.status = 'noShow'
        // }
  ...
  // 使用正则表达式替换掉所有可能的特殊字符
  return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // $&表示与正则表达式匹配的整个字符串
}

  function matchIgnoreCase(str, a) {
    // 首先对字符串a进行转义，以确保特殊字符被正确处理
    const escapedA = escapeRegExp(a);
    // 然后使用转义后的字符串创建正则表达式，'i'标志表示忽略大小写
    const pattern = new RegExp(escapedA, 'i');
    
    // 测试字符串str中是否存在与正则表达式匹配的部分
